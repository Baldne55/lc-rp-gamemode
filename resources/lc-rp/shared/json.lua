...
-- Shared by both server and client (loaded via meta.xml in both sections).
...
local function encode_table(val, stack)
    ...
    stack[val] = nil
    return rawget(val, 1) ~= nil and ("[" .. table.concat(res, ",") .. "]") or ("{" .. table.concat(res, ",") .. "}")
end
...
local function encode_number(val)
    if val ~= val or val <= -math.huge or val >= math.huge then error("unexpected number value") end
...
local function decode_error(str, idx, msg)
    ...
    for i = 1, idx - 1 do col_count = col_count + 1 if str:sub(i, i) == "\n" then line_count = line_count + 1 col_count = 1 end end
...
local function parse_string(str, i)
    ...
    while j <= #str do
        ...
        elseif x == 92 then
            ...
            local c = str:sub(j, j)
            if c == "u" then
                local hex = str:match("^[dD][89aAbB]%x%x\\u%x%x%x%x", j + 1) or str:match("^%x%x%x%x", j + 1) or decode_error(str, j - 1, "invalid unicode escape")
                res = res .. parse_unicode_escape(hex) j = j + #hex
            else
                if not escape_chars[c] then decode_error(str, j - 1, "invalid escape char") end
...
local char_func_map = {
    ['"'] = parse_string, ["0"] = parse_number, ["1"] = parse_number, ["2"] = parse_number, ["3"] = parse_number,
    ["4"] = parse_number, ["5"] = parse_number, ["6"] = parse_number, ["7"] = parse_number, ["8"] = parse_number,
    ["9"] = parse_number, ["-"] = parse_number, ["t"] = parse_literal, ["f"] = parse_literal, ["n"] = parse_literal,
...
-- Expose globally for use by other client scripts (no require in the game client)
JSON = json
